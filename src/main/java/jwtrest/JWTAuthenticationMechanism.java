...
//    @Inject private LoginBean loginBean;
    ...
    public AuthenticationStatus validateRequest(HttpServletRequest request, HttpServletResponse response, HttpMessageContext context) {
       ...
       try{ 
        if((request.getSession().getAttribute("logged-group")==null) && (request.getRequestURI().contains("/admin/") || request.getRequestURI().contains("/verificationCenter/") || request.getRequestURI().contains("/user/")))
            {
...
//                            response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "You are Not Authorised to view this Page . Go Back To Login");
       ...
       catch(Exception elogged)
       {
        ...
        if (token==null && loginBean.getUsername()!=null) {
            ...
            System.out.println("Auth Result :- " + result.getStatus().toString());
...
//        }
//        return null;
        if(context.getRequest().getSession().getAttribute("token") != null)
        {
